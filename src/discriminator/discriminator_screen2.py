...
def build_model(name, train_expert_content=None, train_agent_content=None):
    from discriminator_candidates import build_candidate
    kwargs = {}
    if name.startswith("feature-pruned"):
        kwargs = {"train_expert_content": train_expert_content,
                 "train_agent_content": train_agent_content}
    return build_candidate(name, **kwargs)
...
def run(name, gamma_int, seed, data):
    ...
    model = build_model(name, train_expert_content=train_feat[:, :137],
                       train_agent_content=mappo_train[:, :137])
    ...
    recovered_dir = os.path.join(RESULT_DIR, "recovered")
    os.makedirs(recovered_dir, exist_ok=True)
    ckpt_path = os.path.join(recovered_dir, f"{name}_g{gamma_int:g}_seed{seed}.pt")
    torch.save({
        "model_state_dict": model.state_dict(),
        "candidate": name,
        "gamma_int": gamma_int,
        "seed": seed,
        "final_did": float(final)
    }, ckpt_path)
