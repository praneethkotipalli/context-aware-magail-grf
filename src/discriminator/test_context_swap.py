...
vec = np.random.uniform(-1, 1, size=139).astype(np.float32)
...
# late-losing target: deep in T_norm<0.3, comfortably losing
swapped2 = swap_context(vec, t_norm=0.1, delta_score_raw=-2)
ctx2 = swapped2[BLOCK_SLICES['context']]
print(f"late/loss target -> context = {ctx2}  (expect [0.1, -0.6667])")
