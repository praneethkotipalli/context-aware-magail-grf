"""
build_agent_dataset_mappo.py

Mirrors build_agent_dataset_random_policy.py exactly, pointed at frozen-
MAPPO rollouts instead. Same taxonomy (imported, not redefined), same
cache format, so BalancedContextSampler wraps it identically.
"""
...
CACHE_PATH = os.path.join(os.path.dirname(__file__), "mappo_features_cache.npz")


"""def run():
    paths = sorted(glob.glob(os.path.join(EPISODES_DIR, "*.npz")))
    print(f"Building frozen-MAPPO feature cache from {len(paths)} episodes...")
...
def run():
    paths = sorted(glob.glob(os.path.join(EPISODES_DIR, "*.npz")))
    print(f"Building agent/random feature cache from {len(paths)} episodes...")
